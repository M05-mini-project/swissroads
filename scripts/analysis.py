...
def create_data_sets(df):
    ...
    df_train.loc[:,'category_num'] = df_train.loc[:,'category'].apply(baseline.cat2num)
    df_valid.loc[:,'category_num'] = df_valid.loc[:,'category'].apply(baseline.cat2num)
...
def main(df, epochs = 15):
    ...
    train_acc, test_acc, y_te_predict  = create_execute_TF_graph(epochs, X_tr, X_te, X_va, y_tr, y_te, y_va)   
    print('------------------------------------------------')
