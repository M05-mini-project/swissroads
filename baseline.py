...
def cat2num(x):
    """ 
    Return the int value of a given category

    Parameters
    ----------
    x : string
        value can either be: 'bike' 'car' 'motorcycle' 'other' 'truck' 'van'

    Returns:
    ----------
    int
        value from 0 to 5 included corresponding to the int equivalent of the category input
    """
    if x == "bike":
        return 0
    if x == "car":
...
def main(df):
    ...
    The function will provide system output for its statistics, including a confusion matrix

    Parameters
    ----------
    df : pandas.DataFrame
        The dataframe generated by data_exploration.main()

    Returns:
    ----------
    baseline_acc_tr : float
        The accuracy of the training set
    baseline_acc_te : float
    ...
    features_names = df_train.drop(
        ["imageSet", "category", "imageFileName"], axis=1
    ).columns
    ...
    pipe = Pipeline(
        [
            ...
            (
                "logreg",
                LogisticRegression(
                    multi_class="ovr", solver="liblinear", random_state=0
                ),
            ),
        ]
    )
    ...
    df_res = pd.DataFrame(
        cm, columns=["bike", "car", "motorcycle", "other", "truck", "van"]
    )
